#!/bin/bash
# run: export TEMPLATE=...; export WORKPATH=...; proteinMPNN.slurm.sh
...
##SBATCH --gres=gpu:1
#SBATCH --priority=0
#SBATCH --export=WORKPATH,TEMPLATE
...
# -------------------------------------------------------
# ACTUAL COMMAND
#export WORKPATH=${WORKPATH:-$HOME2/GCN2/output}
...
do
    ...
    #[ ! -f "$WORKPATH/chains_definitions.jsonl" ] && python $HOME2/GCN2/repo/code/prep_MPNN_v3.py "$WORKPATH"
    echo 'Preparing...'$SERIALNUMBER
    python prep_MPNN_mod.py --input-folder "$WORKPATH" --suffix "$SERIALNUMBER" --limit 50;
    echo 'Running...'
